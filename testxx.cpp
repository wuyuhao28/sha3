...
void* getretthread( void* deviceids ){
  get_rets();
}
...
void* ThreadUser( void* deviceids ) {
    ...
    uint8_t len=5;
    uint64_t nonce=123;
    get_hash(g_msg,testseeds,&deviceidin,testseeds,jobid,&len,&nonce);//剔除第一次的时间
    // start = clock();
    for (int i = 0;i<10000; i++) {
      ...
      // int diff=get_hash(g_msg,testseeds,&tmp);
      int diff=get_hash(g_msg,testseeds,&tmp,testseeds,jobid,&len,&nonce);
...
int main(void)
{
  ...
  get_hash_init();
  pthread_t getrettid;
  int ret = pthread_create( &getrettid, NULL, getretthread, NULL);
  if( ret!= 0 ) 
  {  
      std::cout << "pthread_create getret error:error_code=" << ret<< std::endl;  
  } 
  int devicenum=6;
  for( int i = 0; i < devicenum; ++i )  
  {  
      ret = pthread_create( &tids[i], NULL, ThreadUser, (void*)&device[i] );
      if( ret!= 0 ) 
      {  
