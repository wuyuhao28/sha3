...
struct AlgriMatList {
    ...
    AlgriMatList() {
		memset(matVec, 0, sizeof(Mat256x256i8) * 256);
        for(int i=0; i<256; i++) {
            //Mat256x256i8* ptr = new Mat256x256i8;
            //assert(ptr!=NULL);
            //matVec.push_back(ptr);
			matVec[i].toIdentityMatrix();
        }
    }
...
extern AlgriMatList* matList_int8;

//static inline void iter(
//                        const uint8_t *msg,
//                        uint32_t len,
//                        uint8_t result[32]) {
...
static inline void incrNum(uint8_t num[8]) {
    for(int i=0; i<8; i++) {
        if(num[i]!=255) {
            num[i]++;
            break;
